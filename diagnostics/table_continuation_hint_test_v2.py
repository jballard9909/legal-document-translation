"""
table_continuation_hint_test_v2.py

THROWAWAY DIAGNOSTIC -- not a pipeline component.  (v2 of the continuation probe)

v2 DELTA vs v1
--------------
v1 found: "full" style = 6/6 clean rows both directions; "headerless" content was
right but the model often dropped the OUTER pipes (rows came back "a | b", not
"| a | b |"), which md_render's _is_pipe_row / _looks_like_table cannot see.
This v2 (a) tightens the headerless hint to REQUIRE outer pipes on every row, and
(b) focuses the run on the one flaky cell -- en>tr headerless (the production
direction) -- with more reps and an every-rep raw dump. The scorer's outer-pipe
requirement is now the INTENDED pass criterion, not an artifact: a row without
outer pipes is a real fail because the renderer drops it exactly as the scorer
does. (Flip DIRECTIONS/STYLES in run() back to the full matrix to re-confirm.)
...
REPS = 5            # more samples -- we're judging reliability of one focused cell
...
def build_hint(style, header):
    ...
    if style == "headerless":
        directive = (
            ...
            "table rows, one row per record. EVERY row MUST begin with a pipe, "
            "end with a pipe, and separate its two cells with a pipe -- exactly "
            "in this shape: | first cell | second cell |. A row that is not "
            "wrapped in outer pipes is invalid; do not emit one. Do NOT emit a "
...
def run():
    # v2 focus: only the cell that was flaky in v1. "full" was 6/6 both
    # directions and tr>en headerless was clean (the one tr>en zero was a 503
    # outage, not the model), so re-burning calls on settled cells buys nothing.
    # Restore ("headerless", "full") / ("en>tr", "tr>en") to re-run everything.
    styles = ("headerless",)
    directions = ("en>tr",)
    ...
    # --- raw dump: EVERY rep, so any residual flake is visible, not hidden ---
    print("\nRAW OUTPUT (every rep) -- inspect outer pipes + fused-row split + cell text:")
    for direction in directions:
        for style in styles:
            for i, r in enumerate(results[(direction, style)]):
                print("\n" + "-" * 72)
                print(f"[{direction} / {style}] rep {i + 1} raw:")
                print(r["raw"])
